...
def test_clean_orders_normalizes_accepted_values(valid_orders):
    result = clean_orders(valid_orders, validate_orders(valid_orders))
...
def test_clean_orders_quarantines_multiple_quality_issues(valid_orders):
    ...
    data = pd.concat([valid_orders, bad], ignore_index=True)
    result = clean_orders(data, validate_orders(data))
...
def test_clean_orders_isolates_duplicate_rows_and_order_ids(valid_orders):
    ...
    assert validation.issue_counts["duplicate_row"] == 2
    assert validation.issue_counts["duplicate_order_id"] == 2
